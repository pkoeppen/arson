jQuery(document).ready(function($) {
  var config = {
    ...
    'content-api-key': '1c8a269cc7842e751b0b61c7cc'
  };
  ...
  if (typeof Cookies.get('arson-read-later') !== 'undefined') {
    readLaterPosts = JSON.parse(Cookies.get('arson-read-later'));
  }
  ...
  $(window).on('load', function() {
    ...
    setGalleryRatio();
      ...
      swiperPosts.on('slideChangeTransitionEnd', function() {
      ...
      // If body has class 'paged', load next or previous posts based on the current page number.
    ...
    function imageInDiv() {
      $('.swiper-slide:not(.swiper-slide-active) .article-container .post-content img').each(function() {
    ...
    function readLater(content, readLaterPosts) {
      ...
      $(content).find('.read-later').each(function() {
        $(this).on('click', function(event) {
          ...
          $('.read-later[data-id="'+ id +'"]').each(function() {
          ...
          Cookies.set('arson-read-later', readLaterPosts, { expires: 365 });
          bookmarks(readLaterPosts);
        });
      });
    ...
    function bookmarks(readLaterPosts) {
      ...
      if (readLaterPosts.length) {
        ...
        const filter = `id:[${readLaterPosts.toString()}]`;
        ...
        .then(function (results) {
          ...
          $.each(results, function(index, val) {
            ...
            const date = moment(`${dateSplit[2]}-${month}-${dateSplit[1]}`, 'DD-MM-YYYY').format('DD MMM YYYY');
            if (val.primary_tag) {
              $(`.bookmark-container ul[data-tag="${val.primary_tag.name}"]`)
              .append(`<li><time>${date}</time><a href="#" class="read-later active" data-id="${val.id}"></a><a href="${val.slug}">${val.title}</a></li>`);
            } else {
              $('.bookmark-container ul[data-tag="Other"]')
              .append(`<li><a href="#" class="read-later active" data-id="${val.id}"></a><time>${date}</time><a href="${val.slug}">${val.title}</a></li>`);
            }
          });

          $('.bookmark-container').find('.read-later').each(function() {
            $(this).on('click', function(event) {
              event.preventDefault();
              var id = $(this).attr('data-id');
              if ($(this).hasClass('active')) {
